<template>
	<view class="container">
		...
		<map id="maps" class="tui-maps" :longitude="longitude" :latitude="latitude" 
		:scale="16" show-location @regionchange="regionchange" :style="{height:winHeight}">
			<cover-image class="cover-image" src="/static/maps/location.png" />
		</map>
...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap(index) {
			if (this.$u.test.isEmpty(this.province) || this.$u.test.isEmpty(this.city) || this.$u.test.isEmpty(this.area)) {
				this.$u.toast(this.address);
			}else{
				uni.$emit(this.UPDATEADDRESS,{province:this.province,city:this.city,area:this.area, address:this.address} );
				uni.navigateBack();
			}
		},
		onLoad() {
